...
class CDReader {
  ...
  public:
    ...
    void setBuffer(char *buff) { buffer = buff; }
...
class CHDReader : public CDReader {
  private:
    ...
    bool parseTrackMetadata() {
        ...
        err =
            chd_get_metadata(chd, CDROM_TRACK_METADATA2_TAG, 0, metadata, sizeof(metadata), &metalen, nullptr, nullptr);
        if (err == CHDERR_NONE) {
            int tracknum;
            char type[32], subtype[32], pgtype[32], pgsub[32];
            int frames, pregap, postgap;
            if (sscanf(metadata, CDROM_TRACK_METADATA2_FORMAT, &tracknum, type, subtype, &frames, &pregap, pgtype,
                       pgsub, &postgap) >= 4) {
                trackFrames = frames;
                parseTrackType(type);
                return true;
        ...
        err =
            chd_get_metadata(chd, CDROM_TRACK_METADATA_TAG, 0, metadata, sizeof(metadata), &metalen, nullptr, nullptr);
        if (err == CHDERR_NONE) {
            int tracknum;
